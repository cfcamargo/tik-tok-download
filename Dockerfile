...
# Instala python3, pip, python3-venv (para venv) e ffmpeg
RUN apt-get update && \
...
# === 2. INSTALAÇÃO DO YT-DLP EM AMBIENTE VIRTUAL E MOVER PARA O PATH ===
...
# === 5. HEALTHCHECK E COMANDO (Corrigido para Coolify) ===
# Healthcheck: Verifica a cada 10s se o processo 'node' principal ainda está ativo.
HEALTHCHECK --interval=10s --timeout=5s --start-period=10s \
  CMD ["sh", "-c", "ps aux | grep -v grep | grep node > /dev/null || exit 1"]
